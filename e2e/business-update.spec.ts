...
test.describe('Business Update Flow', () => {
  ...
  test('user can update business profile', async ({ page }) => {
    ...
    await expect(page.getByText('Loading business data...')).not.toBeVisible({ timeout: 10000 })
    ...
    await expect(page.getByRole('button', { name: 'Edit Profile' })).toBeVisible() // Edit button returns
    ...
    await expect(page.getByLabel('Business Description')).toHaveValue(newDescription)
